...
- Workspace-only registry repos are cloned and pulled by the global bootstrap but create **no home-global discovery links**. Add their canonical skill directory to `$HOME/.agents/workspace-skills.local.md` so the workspace reconciler exposes them only from the intended opened root.
...
Registry entries use `- Path: \`...\``, `- GitHub: \`owner/repo\``, and `- Symlink scheme: ...`; supported schemes are `agent-neutral`, `claude-only`, `codex-only`, and `workspace-only`. They may also include `- Skills path: \`<subdir>\``. When `Skills path` is omitted, the repo is treated like the personal repo: top-level directories containing `SKILL.md` are skills. When it is `skills`, top-level directories under that subdirectory are skills. When it is `.`, the repository root itself is one skill, and the link name is the repository directory name. Every `workspace-only` entry must also have a matching `Skills` entry in `$HOME/.agents/workspace-skills.local.md`; this keeps clone/pull automation while delegating discovery to the workspace projection layer.
...
5. Ensure the discovery links match the repo's scheme: agent-neutral → `$HOME/.agents/skills` and `$HOME/.claude/skills`; cc-skills → `$HOME/.claude/skills` only; codex-skills → `$HOME/.codex/skills` only; workspace-only → no home-global link, with root-local links managed through the workspace projection registry. Remove stale links from directories outside the selected scheme.
...
- The skill has links only in its scheme's discovery directories: agent-neutral → `$HOME/.agents/skills` and `$HOME/.claude/skills`; Claude Code-only → `$HOME/.claude/skills`; Codex-only → `$HOME/.codex/skills`; workspace-only → no home-global links and both links at every registered opened root.
